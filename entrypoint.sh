...
if [ "$1" = 'kibana' ]; then
	if [ "$ELASTICSEARCH_URL" -o "$ELASTICSEARCH_PORT_9200_TCP" ]; then
		...
		sed -ri "s!^(elasticsearch.url:).*!\1 '$ELASTICSEARCH_URL'!" config/kibana.yml
	fi

	if [ "$WEBITEL_PASS" ]; then
		sed -ri "s!^(webitel.security.password:).*!\1 '$WEBITEL_PASS'!" config/kibana.yml
	fi

if [ "$ENGINE_AUTH_URL" -o "$ENGINE_AUTH_PORT_10022_TCP" ]; then
		: ${ENGINE_AUTH_URL:='http://engine:10022'}
		sed -ri "s!^(webitel.main.engineAuthUri:).*!\1 '$ENGINE_AUTH_URL'!" config/kibana.yml
	fi

if [ "$ENGINE_URL" -o "$ENGINE_PORT_10022_TCP" ]; then
		: ${ENGINE_URL:='http://engine:10022'}
		sed -ri "s!^(webitel.main.engineUri:).*!\1 '$ENGINE_URL'!" config/kibana.yml
	else
...
if [ "$FS_URL" -o "$FS_PORT_8082_TCP" ]; then
		...
		sed -ri "s!^(webitel.main.webRtcUri:).*!\1 '$FS_URL'!" config/kibana.yml
	else
